/*
 ...
 |__   __|        |  \/  (_)         | |  | |     | |    
    | | ___  _ __ | \  / |_ _ __ __ _| |__| | ___ | |____
    | |/ _ \| '_ \| |\/| | | '__/ _` |  __  |/ _ \| |_  /
    | | (_) | | | | |  | | | | | (_| | |  | | (_) | |/ / 
    |_|\___/|_| |_|_|  |_|_|_|  \__,_|_|  |_|\___/|_/___|
                                                         
                                                         
    TonUINO Version 2.1 - TonUINO für Miras Holz-Box
...
mySettings.shortCuts[0].folder = 3; // Ordner bei langem Druck auf mittlerer Taste abspielen
  mySettings.shortCuts[0].mode = 3;   // Party-Modus
  mySettings.shortCuts[1].folder = 4; // Ordner bei langem Druck auf rechter Taste abspielen
  mySettings.shortCuts[1].mode = 3;   // Party-Modus
  mySettings.shortCuts[2].folder = 2; // Ordner bei langem Druck auf linke Taste abspielen
  mySettings.shortCuts[2].mode = 3;   // Party-Modus
  mySettings.shortCuts[3].folder = 1; // Odner beim Start abspielen
  mySettings.shortCuts[3].mode = 3;   // Party-Modus
...
static void nextTrack(uint16_t track) {
  if (activeModifier != NULL)
   ...
   if (_lastTrackFinished == 9999) {
      // Wenn Albumnummer angesagt wird, kein nextTrack()    <<<< Abweichung zu TonMobilLED
      return;
   }
...
void setup() {
  ...
  Serial.println(F("  _______          __  __ _           _    _       _     "));
  Serial.println(F(" |__   __|        |  \/  (_)         | |  | |     | |    "));
  Serial.println(F("   | | ___  _ __ | \  / |_ _ __ __ _| |__| | ___ | |____ "));
  Serial.println(F("    | |/ _ \| '_ \| |\/| | | '__/ _` |  __  |/ _ \| |_  /"));
  Serial.println(F("    | | (_) | | | | |  | | | | | (_| | |  | | (_) | |/ / "));
  Serial.println(F("    |_|\___/|_| |_|_|  |_|_|_|  \__,_|_|  |_|\___/|_/___|"));
  Serial.println(F(" "));
  Serial.print(F("TonUINO für Miras Holz-Box Version ")); Serial.println(version);
...
void playShortCut(uint8_t shortCut) {
  Serial.print(F(">>>> Spiele nun Shortcut["));
  Serial.print(shortCut);
  if (mySettings.shortCuts[shortCut].folder != 0) {
    myFolder = &mySettings.shortCuts[shortCut];
    Serial.print("] Ordner: "); 
    Serial.print(mySettings.shortCuts[shortCut].folder);
    Serial.print(F(" Modus: ")); 
    Serial.println(mySettings.shortCuts[shortCut].mode);
    playFolder();
    disablestandbyTimer();
    delay(1000);
  }
