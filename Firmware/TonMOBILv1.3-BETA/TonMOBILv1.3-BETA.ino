...
void setup() {
  ...
  // Standardwerte aus EEPROM laden
  ...
  delay(3000);  // Drei Sekunden warten bis der DFPlayer Mini sicher initialisiert ist
  ...
  if (tempCard.cookie == cardCookie) {
    ...
    if (tempCard.nfcFolderSettings.folder == 0) {
      ...
      switch (tempCard.nfcFolderSettings.mode ) {
        case 0:
        case 255: mfrc522.PICC_HaltA(); mfrc522.PCD_StopCrypto1(); adminMenu(true);  break;
...
void writeCard(nfcTagObject nfcTag) {
  ...
  if ((mifareType == MFRC522::PICC_TYPE_MIFARE_MINI ) || (mifareType == MFRC522::PICC_TYPE_MIFARE_1K ) || (mifareType == MFRC522::PICC_TYPE_MIFARE_4K ) ) {
    ...
    byte pACK[] = {0, 0}; //16 bit PassWord ACK returned by the NFCtag Authenticate using key A
