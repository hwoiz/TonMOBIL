...
#define LED_BRIGTNESS 40          // Helligkeit 0-255
...
void setup() {
  ...
  strip.fill(strip.Color(LED_BRIGTNESS,0,0), 0, 0);
  strip.show();
  loopCountdown = 0;
...
#ifdef LED_SR
  // Statusanzeige während auf DFPlayer gewartet wird
  strip.clear();
  i = 1;
  do {
    strip.fill(strip.Color(LED_BRIGTNESS,LED_BRIGTNESS,0), 0, i);
    strip.show();
    delay(200);
    i++;
  } while (i < LED_COUNT);
#endif
#ifndef LED_SR
  //Warten auf DFPlayer-Init wenn keine Statusanzeige vorhanden
  delay(3000);
#endif
...
#ifdef LED_SR
  strip.fill(strip.Color(0,LED_BRIGTNESS,0), 0, 15);
  strip.show();
#endif
